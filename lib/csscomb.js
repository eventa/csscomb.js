...
Comb.prototype = {
    ...
    processFile: function(path) {
        ...
        if (this._shouldProcess(path)) {
            // Get file's extension and check if syntax is supported:
            var syntax = path.split('.').pop();
            if (_this.SUPPORTED_SYNTAXES.indexOf(syntax) < 0) {
                // Print to console instead of throwing errors to leave output
                // clean:
                console.error('Sorry, csscomb does not support *.' + syntax +
                    ' files yet.');
                return null;
            }
